...
export default function DescargasPlanosPage() {
  const [query, setQuery] = useState("");

  // Filtrar por categoría "planos"
  const recursosPlanos = useMemo(
    () => descargas.filter((r) => r.categoria === "planos"),
    []
  );

  // Filtro por texto
  const recursosFiltrados = useMemo(() => {
    if (!query.trim()) return recursosPlanos;
    const q = query.toLowerCase();
    return recursosPlanos.filter(
      (r) =>
        r.titulo.toLowerCase().includes(q) ||
        r.descripcion.toLowerCase().includes(q)
    );
  }, [recursosPlanos, query]);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <header className="mb-6">
          ...
          <p className="text-sm text-gray-700 mt-2">
            Planos DWG, perfiles longitudinales, secciones, documentación gráfica
            y material geoespacial para obras ferroviarias.
          </p>
        </header>
        ...
        <section className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 md:p-6">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-4">
            <div className="text-xs text-gray-500">
              {recursosFiltrados.length} plano
              {recursosFiltrados.length === 1 ? "" : "s"} encontrado
              {recursosFiltrados.length === recursosPlanos.length
                ? ""
                : ` (de ${recursosPlanos.length} totales)`}
            </div>
          ...
          {recursosFiltrados.length === 0 ? (
            <p className="text-sm text-gray-500">
              No se encontraron planos que coincidan con tu búsqueda.
            </p>
          ) : (
